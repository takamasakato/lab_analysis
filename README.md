...
本リポジトリは、自身が行った、in vivoカルシウムイメージングデータを対象とした、
...
OpenCVを用いた、小脳プルキンエ細胞活動のクラスタリング解析例  
（時空間解析とクラスター解析の統合）
...
## 実装範囲（My Contribution）
・CaImAnを用いた動き補正および自動細胞抽出の実装  
・ΔF/F前処理から統計評価までの解析フロー設計  
・刺激応答の定量評価（Glass’s Δ）実装  
・ROI位置情報と神経活動の統合による空間マッピング  
・OpenCVを用いた時空間クラスタリング解析 
